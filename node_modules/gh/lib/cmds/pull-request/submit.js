...
async function submit(options, user) {
    ...
    if (utils_1.userLeftMsgEmpty(title)) {
        title = utils_1.openFileInEditor('temp-gh-pr-title.txt', `# Add a pr title message on the next line\n${git.getLastCommitMessage(pullBranch)}`);
    }
    ...
    if (utils_1.userLeftMsgEmpty(options.title) || utils_1.userLeftMsgEmpty(description)) {
        description = utils_1.openFileInEditor('temp-gh-pr-body.md', '<!-- Add an pr body message in markdown format below -->');
    }
    const payload = Object.assign({ mediaType: {
            previews: ['shadow-cat'],
