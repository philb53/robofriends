...
async function run(options, done) {
    ...
    const files = (await base.find(cmdDir).promise()).filter(name => !name.includes('help') && !name.includes('version') && !name.includes('.map'));
    ...
    const commands = await Promise.all(files.map(async (fileName) => {
        let cmd = await Promise.resolve().then(() => require(path.resolve(cmdDir, fileName)));
        let flags = [];
        if (cmd.Impl) {
            cmd = cmd.Impl;
        }
